...
function SongsNav({ state, dispatch }: SideNavProps): JSX.Element {
  ...
  */
  ...
  const [songItem, setSongItem] = React.useState('');

  const filteredSong = (event: React.ChangeEvent<HTMLInputElement>) => {
    const songTerm = event.target.value;
    setSongItem(songTerm);
  };
    ...
    <Section title="Playlist">
      <>
        <input
          type="text"
          value={songItem}
          onChange={filteredSong}
          placeholder="Search..."
          className="f6 pointer flex items-center"
          style={{
            borderRadius: '0.4rem',
            marginBottom: '1rem'
          }}
